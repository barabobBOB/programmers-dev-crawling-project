...
// import { getCoinSymbolNewsList } from './symbolNews';

function CoinList({ getOneCoinData, setClickedSymbol}) {
    ...
    async function fetchCoinList() {
      try {
        const response = await axios.post(
          'http://127.0.0.1:8000/coin/api-v1/coinsync',
          { is_sync: 'True' },
        );
  ...
  async function handleSync() {
    ...
    for (let i = 0; i < coinList.length; i += 3) {
      ...
      const coinGroupPromises = coinGroup.map((coinSymbol) =>
  ...
  return (
    <div className="col-lg-1 col-xl-4">
      <div
        ...
        style={{ height: '415px', width: '500px' }}
      >
